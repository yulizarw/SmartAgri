const { WeatherForecast, Farm } = require("../models");
...
class WeatherForecastService {
  ...
  static async saveWeather(farmId, date) {
    // =========================================
    // 1. VALIDASI FARM
    // =========================================
    ...
    const weather = await GeeService.analyzeWeather(farmId, date);

    console.log("Weather dari GEE:", weather);
    ...
    if (existing) {
      await existing.update(weatherData);
    ...
    const result = await WeatherForecast.create(weatherData);
